  ...
  <div class="page-container">
    <el-card class="search-card" shadow="never">
      <div class="toolbar">
        ...
        <el-button type="primary" :icon="Plus" @click="openDialog()">新增采购单</el-button>
      </div>
    </el-card>
    ...
    <el-table :data="tableData" stripe v-loading="loading">
      ...
      <el-table-column label="状态" width="100">
        <template #default="{ row }">
          <el-tag v-if="row.status === PURCHASE_ORDER_STATUS.DRAFT" size="small">草稿</el-tag>
          <el-tag v-else-if="row.status === PURCHASE_ORDER_STATUS.PENDING" type="warning" size="small">待审核</el-tag>
          <el-tag v-else-if="row.status === PURCHASE_ORDER_STATUS.APPROVED" type="success" size="small">已通过</el-tag>
          <el-tag v-else-if="row.status === PURCHASE_ORDER_STATUS.REJECTED" type="danger" size="small">已拒绝</el-tag>
          <el-tag v-else-if="row.status === PURCHASE_ORDER_STATUS.INBOUNDED" type="success" size="small">已入库</el-tag>
      ...
      <el-table-column label="操作" width="320" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === PURCHASE_ORDER_STATUS.DRAFT" size="small" type="primary" link @click="handleSubmit(row)">提交</el-button>
          <el-button v-if="row.status === PURCHASE_ORDER_STATUS.APPROVED" size="small" type="primary" link @click="handleConfirmInbound(row)">确认入库</el-button>
          <el-button v-if="row.status === PURCHASE_ORDER_STATUS.PENDING" size="small" type="success" link @click="handleApprove(row)">审核</el-button>
          <el-button v-if="row.status === PURCHASE_ORDER_STATUS.PENDING" size="small" type="danger" link @click="handleReject(row)">拒绝</el-button>
          <el-button v-if="row.status === PURCHASE_ORDER_STATUS.DRAFT || row.status === PURCHASE_ORDER_STATUS.REJECTED" size="small" type="danger" link @click="handleDelete(row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <!-- 新增采购单对话框 -->
    <el-dialog v-model="dialogVisible" title="新增采购单" width="500px" destroy-on-close>
    ...
    <!-- 查看明细对话框 -->
    <el-dialog v-model="detailVisible" title="采购单明细" width="600px" destroy-on-close>
...
async function handleSave() {
  if (!form.supplierId || !form.warehouseId) {
    ElMessage.warning('请填写供应商ID和仓库ID')
    return
  }
  saving.value = true
  try {
...
async function viewItems(row: any) {
  ...
  try {
    orderItems.value = await erpApi.getPurchaseOrderItems(row.id)
  } catch {
    orderItems.value = []
...
async function handleConfirmInbound(row: any) {
  try {
