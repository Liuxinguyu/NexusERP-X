...
export default function OpportunityManage() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      const body: Partial<OpportunityRow> = { opportunityName: name, amount: amount ? Number(amount) : undefined, remark }
      if (editId) await opportunityApi.update(editId, body)
      else await opportunityApi.create(body)
      setFormOpen(false); toast.success(editId ? '商机更新成功' : '商机创建成功'); void loadData()
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <tr key={row.id} className="hover:bg-slate-50 transition-all">
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex gap-3 justify-end">
                      ...
                      {nextStage && <button onClick={() => handleAdvanceStage(row.id!, nextStage)} className="text-emerald-600 text-xs font-black hover:underline">推进→{nextStage}</button>}
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={editId ? '编辑商机' : '新建商机'}>
