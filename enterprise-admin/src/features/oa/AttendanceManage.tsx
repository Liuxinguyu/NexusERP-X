  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [today, recs, st] = await Promise.all([
        attendanceApi.myToday().catch(() => ({}) as TodayStatus),
        attendanceApi.recordsPage({ current: 1, size: 10 }).then(r => pickPageRecords(r)).catch(() => []),
        attendanceApi.monthlyStatistics({ year: now.getFullYear(), month: now.getMonth() + 1 }).catch(() => ({}) as AttendanceStatistics),
      ])
      ...
      <div className="col-span-4 bg-white p-12 rounded-[3.5rem] shadow-sm ring-1 ring-slate-100 flex flex-col items-center">
        ...
        {todayStatus.checkInTime && (
          <div className="mt-8 text-center">
            ...
            <div className="text-sm font-black text-emerald-600 mt-1">{todayStatus.checkInTime}</div>
            {todayStatus.checkOutTime && <div className="text-sm font-black text-emerald-600">{todayStatus.checkOutTime}</div>}
          </div>
        )}
              ...
              {records.map(r => (
                <div key={r.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-[1.5rem] hover:bg-white hover:ring-1 ring-slate-200 transition-all">
                  <div className="flex gap-4 items-center">
                    ...
                    <div>
                      <div className="text-[10px] font-black text-slate-300 uppercase">{r.date ?? '-'}</div>
                      <div className="text-sm font-black text-slate-900">{r.checkInTime ?? '-'} — {r.checkOutTime ?? '-'}</div>
                    </div>
                  </div>
