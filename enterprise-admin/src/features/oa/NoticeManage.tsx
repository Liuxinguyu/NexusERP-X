...
export default function NoticeManage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await noticeApi.page({ pageNum: current, pageSize: size })
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      if (editId) await noticeApi.update(editId, { title, content })
          ...
          <tbody className="divide-y divide-slate-100">
            {loading && !list.length ? (
              ...
              <tr key={row.id} className="hover:bg-slate-50 transition-all">
                ...
                <td className="px-8 py-6 text-slate-400 text-xs">{row.createTime ?? '-'}</td>
                <td className="px-8 py-6 text-right">
                  <div className="flex gap-3 justify-end">
                    <button onClick={() => openEdit(row)} className="text-indigo-600 text-xs font-black hover:underline">编辑</button>
                    {row.status !== 1 && <button onClick={() => handlePublish(row.id!)} className="text-emerald-600 text-xs font-black hover:underline">发布</button>}
                  </div>
                </td>
              </tr>
            ))}
