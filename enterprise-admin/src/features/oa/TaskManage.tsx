  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <PermGate perms={[OA_PERMS.task.add]}>
          ...
          <div key={col.status} className={`bg-white rounded-[2.5rem] shadow-sm ring-1 ring-slate-100 overflow-hidden border-t-4 ${col.color}`}>
            ...
            <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto">
              {all.filter(t => String(t.status) === col.status).map(task => (
                <div key={task.id} onClick={() => void openDetail(task)}
                  ...
                  {task.deadline && <div className="text-[10px] text-slate-300">{formatDateTime(task.deadline)}</div>}
                  <div className="flex gap-2 mt-3">
                    {col.status === '0' && <button onClick={e => { e.stopPropagation(); void handleAction(task.id!, 'accept') }} className="text-[10px] font-black text-indigo-600 hover:underline">接受</button>}
                    {col.status === '1' && <button onClick={e => { e.stopPropagation(); void handleAction(task.id!, 'complete') }} className="text-[10px] font-black text-emerald-600 hover:underline">完成</button>}
                    {(col.status === '0' || col.status === '1') && <button onClick={e => { e.stopPropagation(); void handleAction(task.id!, 'cancel') }} className="text-[10px] font-black text-rose-400 hover:underline">取消</button>}
                  </div>
                </div>
              ))}
          ...
          <div className="border-t border-slate-100 pt-4">
            ...
            {comments.length === 0 ? (
              ...
              <div key={c.id} className="p-3 bg-slate-50 rounded-xl mb-2">
                <div className="text-[10px] font-black text-slate-400">{c.userName} · {formatDateTime(c.createTime)}</div>
            ...
            <div className="flex gap-2 mt-3">
              ...
              <button onClick={() => void handleAddComment()} disabled={commentSubmitting} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-black hover:bg-indigo-500 transition-all disabled:opacity-60">{commentSubmitting ? '发送中...' : '发送'}</button>
            </div>
          </div>
