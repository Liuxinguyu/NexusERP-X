...
export default function FinanceManage() {
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      await api.recordPayment(paymentModal.id, { amount: Number(payAmount), remark: payRemark })
          ...
          <tbody className="divide-y divide-slate-100">
            {loading && !list.length ? (
              ...
              <tr key={row.id} className="hover:bg-slate-50 transition-all">
                ...
                <td className="px-8 py-6 text-slate-500">{row.dueDate ?? '-'}</td>
                <td className="px-8 py-6 text-right">
                  <button onClick={() => setPaymentModal({ id: row.id!, type: tab })}
                    className="text-indigo-600 text-xs font-black hover:underline">{tab === '应收' ? '收款登记' : '付款登记'}</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <Modal open={!!paymentModal} onClose={() => setPaymentModal(null)} title={paymentModal?.type === '应收' ? '收款登记' : '付款登记'}>
        <div className="p-8 space-y-6">
          ...
          <div className="flex gap-4">
            ...
            <button onClick={handleRecordPayment} className="flex-[2] py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs shadow-xl shadow-indigo-200 hover:bg-indigo-500 transition-all">确认</button>
          </div>
        </div>
      </Modal>
    </div>
